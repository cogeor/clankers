...
def pd_controller(obs: np.ndarray) -> float:
    ...
    force = k_pos * cart_pos + k_vel * cart_vel + k_angle * pole_angle + k_angle_vel * pole_vel
...
def main() -> None:
    ...
    if survived >= num_episodes * 0.8:
        ...
        print("  Physics dynamics are consistent with CartPole-v1")
    else:
        print(f"\n  NEEDS TUNING: Only {survived}/{num_episodes} episodes balanced")
        print("  Physics may differ from CartPole-v1 or gains need adjustment")
