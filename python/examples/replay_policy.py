...
def load_onnx_model(path: str) -> tuple:
    """Load ONNX model, return (inference_fn, encoder_or_None)."""
...
def write_comparison_json(
    ...
    data = {
        ...
        "per_step_mse": [
            float(np.mean((gt_positions[i] - pred_positions[i]) ** 2))
            for i in range(n)
        ],
    }
...
def write_replay_trace(
    ...
    for i in range(n):
        ...
        info["predicted_joint_positions"] = dict(
            zip(encoder.names, pred_positions[i].tolist())
        )
        ...
        output_steps.append({
            ...
            "next_obs": (
                pred_positions[i + 1].tolist() if i + 1 < len(pred_positions)
...
def main() -> None:
    ...
    parser.add_argument("--model", required=True,
                        help="Path to .onnx or .pt model")
    parser.add_argument("--trace", required=True,
                        help="Path to ground-truth trace JSON")
    parser.add_argument("--output", default=None,
                        help="Output replay trace JSON (default: <trace>_predicted.json)")
    parser.add_argument("--scene", default=None,
                        help="Scene spec JSON for joint names")
    parser.add_argument("--encoder", default=None,
                        help="Encoder JSON path (required for .pt models without scene)")
    parser.add_argument("--plot", action="store_true",
                        help="Show matplotlib comparison plot")
    args = parser.parse_args()

    # Load model
    model_path = args.model
    if model_path.endswith(".onnx"):
        infer_fn, model_encoder = load_onnx_model(model_path)
        ...
        if args.encoder:
            ...
            return
        infer_fn, model_encoder = load_pt_model(model_path, enc)
    else:
    ...
    # Open-loop rollout
    pred_positions = open_loop_rollout(infer_fn, gt_positions[0], len(gt_positions))
